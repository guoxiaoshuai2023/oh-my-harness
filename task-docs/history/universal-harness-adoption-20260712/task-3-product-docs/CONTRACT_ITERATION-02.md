...
Identity: `universal-harness-adoption-task-3-contract-20260713-v2`
...
## 3. Triggered Routes And Gate Ownership

The executor packet must explicitly bind these controlling sources; route discoverability is not sufficient:

- `docs/agent-routing/task-planning-scale.md`: Task 3 is one frozen documentation delivery unit; reviews and acceptance remain gates, not extra delivery tasks.
- `docs/agent-routing/current-state-evidence.md`: package, CLI, mapping, and accepted Task 1/2 facts must be read from current primary evidence before documentation claims are written.
- `docs/agent-routing/implementation-boundary.md`: only the five exact documentation paths may change; no adjacent cleanup or behavior repair.
- `docs/agent-routing/validation-and-reporting.md`: evidence must identify what changed, what was checked, what was not checked, and remaining risk.
- `task-docs/_harness/adaptive-orchestration-protocol.md`: the main thread owns topology, packet completeness, intervention, synthesis, and final acceptance.
- `task-docs/_harness/semantic-fidelity-protocol.md`: T3-AC6/T3-AC7 and modal-force preservation require semantic evidence, not structural checks alone.
- `task-docs/_harness/run-directory-protocol.md`: the accepted boundary, producer report, independent QA, and final main-thread decision remain separate authorities.
- `docs/agent-routing/external-systems-and-secrets.md` and `docs/agent-routing/high-risk-actions.md` are bound as stop-only gates: registry credentials, publication, signing, release creation, secrets, and all external writes are prohibited in Task 3.

Gate ownership:

- executor: implement only the five documentation paths, cite current primary evidence, run contract validation, and report uncertainty or stop conditions;
- producer-independent Result Evaluator: inspect actual wording/diff and independently decide T3-AC1 through T3-AC9, modal force, scope, and evidence truthfulness without repair;
- main thread: arbitrate findings, inspect primary evidence, make final Task 3 acceptance, and retain all external-write decisions;
- user: no further decision is required for this bounded documentation implementation; any license, registry, credential, signing, publication, or scope-expansion decision requires a new explicit authorization.
...
## 7. Evidence, Validation, And Report
...
- a command/path/authority terminology matrix covering README, architecture, adapter, both new docs, package metadata/CLI usage, and the bound protocol/profile/calibration authorities;
...
- compare documented commands/options with `package.json` and the accepted CLI usage interface. The controlling command is `node bin/oh-my-harness.mjs --help`; its expected result is exit status `2`, empty stdout, and exactly `usage: oh-my-harness <install|update|uninstall> --target <repo> [--dry-run] [--yes]` plus LF on stderr. This is evidence of the existing usage contract, not a successful help feature and not authority to change the CLI;
