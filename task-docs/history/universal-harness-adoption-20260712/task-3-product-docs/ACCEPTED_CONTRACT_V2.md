# Task 3 Revised Verification Contract v8

Identity: `universal-harness-adoption-task-3-contract-20260713-v8`
...
Build/read back and validate CLI usage with the following fail-preserving block. Any build, assertion, CLI, cleanup, or residue failure makes the block fail; cleanup is attempted even after a validation failure. All six README literals are frozen by the cited compatibility contract.
...
(
  ...
  cli_status=0
  node bin/oh-my-harness.mjs >"$tmp/cli.stdout" 2>"$tmp/cli.stderr" || cli_status=$?
  test "$cli_status" -eq 2
  test ! -s "$tmp/cli.stdout"
  test "$(cat "$tmp/cli.stderr")" = 'usage: oh-my-harness <install|update|uninstall> --target <repo> [--dry-run] [--yes]'
) || validation_status=$?
cleanup_status=0
rm -rf "$tmp" || cleanup_status=$?
if [ "$validation_status" -ne 0 ] || [ "$cleanup_status" -ne 0 ] || [ -e "$tmp" ]; then
  exit 1
fi
```
