...
| Resume decision 01 | `universal-harness-adoption-task-2-contract-resume-decision-20260712-v1` | `85d6fc75c904152657a9c9c60057f64c69b1d714600ba2101230fc4107fcea7c` |
| Contract Review 04 | `universal-harness-adoption-task-2-contract-review-20260712-v4` | `9230c09c69375f27677566f65d03c3cd2c1005f633c76ee707c492e821d1de8a` |
| Exhaustion decision 02 | `universal-harness-adoption-task-2-contract-exhaustion-decision-20260712-v2` | `a714bfbc46dc034b36a876051f1661b02ef552b3e57e1b2f893846cb0a641a18` |
| Resume decision 02 | `universal-harness-adoption-task-2-contract-resume-decision-20260712-v2` | `b3765e0f83b8ba1bb2d34dc1975951e9e2786daa038bcddc0216287fe9725043` |
...
Final sentinel is exactly `.oh-my-harness/.operation-in-progress.json`. Target atomic temporaries have fixed exact names, avoiding directory scans: `.oh-my-harness/.operation-in-progress.tmp`, `.oh-my-harness/.install-state.tmp`, and, for update restoration, OS-temporary in-memory/owned copies of the prior state. Payload atomic temporaries are `.<basename>.oh-my-harness-tmp` beside each exact planned file. Any exact recognized temp present at preflight is incomplete state and blocks mutation.
...
2. create `.oh-my-harness/.operation-in-progress.tmp` with exclusive `open(..., "wx")`, write/fsync/close canonical bytes, create `.oh-my-harness/.operation-in-progress.json` exclusively with same-filesystem `link(temp, final)`, verify the final literal path and bytes, then unlink the temp;
...
- `validate:package` is exactly `node test/lifecycle/package-validation.mjs` and is the only package-validation entry;
...
## 5. Fixed Package Bundle And Single Package-Validation Entry

The only package-validation entry is:

```sh
npm run validate:package
```

`package.json` binds it exactly to `node test/lifecycle/package-validation.mjs`. No other script or ad hoc command may claim package-validation completion.

The validator is Node-stdlib-only and implements one failure-preserving sequence:

1. create a fresh OS temporary root outside the repository and record its real path;
2. create isolated `home`, `xdg`, `cache`, `logs`, `tmp`, `archive`, `package`, and `fixtures` children plus empty user/global npmrc files;
3. stage the four source-backed package surfaces and build the fifth allowlisted `dist/**` surface directly under the temporary `package/`; repository `dist/` is never created;
4. spawn npm pack from the staged package using `/usr/bin/env -i`, verified Node 24 PATH, isolated HOME/XDG/cache/log/tmp/config, loopback registry, offline mode, `--ignore-scripts`, and audit/fund/update-notifier/progress disabled;
5. require exactly one archive; inspect its allowlist, package identity, bundle inventory/hashes, and forbidden content;
6. under the same isolated environment, run the canonical local-archive CLI against temporary fixtures and require successful install, same-version install no-op, same-version update no-op, and uninstall; clean cross-version update remains covered by T2-AC7 integration fixtures;
7. compare repository status/residue against the pre-run baseline;
8. in a `finally` block, verify the temp-root real path and owned child set, remove only that root, then verify absence.

Every spawned child uses synchronous status capture. The first nonzero child status becomes `primaryStatus` and later goal-advancing phases stop. The `finally` cleanup always runs. If `primaryStatus` is nonzero, the validator exits with that same status even when cleanup also fails, while reporting cleanup failure separately. If primary phases succeed but cleanup or residue verification fails, exit status is `1`. Signals, missing status, spawn errors, multiple/missing archives, unexpected files, repository residue, or secret/config access are failures.

The npm child environment contains only explicit non-sensitive variables:

```text
PATH=<verified Node24 and npm system paths>
HOME=<temp>/home
...
npm_config_logs_dir=<temp>/logs
npm_config_tmp=<temp>/tmp
...
No inherited npm token, auth, proxy, Git-routing, Node-option, registry, or tracing variable is passed. The staged working directory contains no project npmrc. npm lifecycle scripts are disabled for pack and local-archive install. No registry read/authentication/publication is authorized.

The archive may contain only package metadata/README, binary, installer modules, and generated fixed bundle. Planning/history/tests, state, backups, caches, secrets, source absolute paths, and unscoped installer guidance are forbidden. Before and after validation, repository archive, `dist`, npm cache/config/log, or pack metadata residue is forbidden.
...
The closed canonical plan contains no `planSha256` or other self-hash field. `version` below uses the Section 9 version grammar; `sha256` means a string matching `^[0-9a-f]{64}$`:
...
modifiedManaged: sorted unique [{
  path: repo-relative path
  kind: "payload" | "inventory" | "managed-block"
  expectedSha256: sha256
  currentSha256: sha256
}]
backups: sorted unique [{
  sourcePath: repo-relative path | "AGENTS.md#managed-block"
  backupPath: validated path under ".oh-my-harness-backups/<operation-id>/"
  sha256: sha256
}]
conflicts: sorted unique [{
  code:
    "UNMANAGED_NAMESPACE" |
    "UNOWNED_DESTINATION" |
    "INVALID_MANAGED_MARKER" |
    "UNVERIFIABLE_INSTALL_STATE" |
    "INCOMPATIBLE_INSTALLATION" |
    "DIRTY_OVERLAP" |
    "UNSAFE_PATH" |
    "TARGET_CHANGED" |
    "IO_UNAVAILABLE"
  path: null | repo-relative path
}]
gitOverlap:
...
Hash the complete canonical plan JSON bytes externally with SHA-256. The confirmation/apply envelope is exactly:

```text
schemaVersion: 1
plan: the complete canonical plan object
planSha256: string matching ^[0-9a-f]{64}$
```

The envelope itself is not hashed. `planSha256` must equal lowercase SHA-256 of the plan's complete canonical bytes. Display the envelope before confirmation. Apply receives the envelope as immutable input, regenerates canonical plan bytes after final pre-write recheck, and requires byte equality plus hash equality. Runtime conflict-code extension and `OTHER` are forbidden. All defined lifecycle conflict conditions map to the closed enum. An unexpected internal condition that cannot accurately use one value fails before a valid plan/apply transition, returns exit `5`, and performs no mutation. The plan never contains unrelated target bytes.
...
The process uses `/usr/bin/env -i` and a trusted absolute Git executable resolved once from the installer environment before target discovery. The executable must be a regular executable outside the target root; its absolute path/version is recorded in evidence, and inability to establish that boundary is STOP. The product does not hardcode this workstation's Git path/version. The invocation uses `--no-optional-locks`, literal exact-file pathspecs only, and config/env that disables hooks, fsmonitor, untracked cache, submodule recursion/summary/diff, external diff, prompts, pager, global/system config, and inherited `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, filters, proxies, and tracing. It runs:
...
The final sentinel at `.oh-my-harness/.operation-in-progress.json` uses Section 6 canonical JSON with exactly: `schemaVersion:1`, `operationId`, `operation`, `targetIdentity`, `priorStateSha256:null|sha256`, `requestedVersion:null|version`, and `planSha256:sha256`. The plan hash is over full canonical plan bytes with no self-hash. No additional key is allowed.
...
- `npm run validate:package`, proving isolated pack/archive inspection/local-archive smoke/failure propagation/finally cleanup/residue behavior from the single Section 5 entry;
...
Contract Reviews 01-04 are controlling failure history, not accepted authority. Their supported findings are incorporated into the current normative sections. Earlier proposed mechanisms, including repository-local transient `dist`, direct target `git status`, underspecified sentinel/state commit behavior, compact JSON encoding, and an incomplete package-validation sequence, are superseded by this v5 contract and must not be implemented.
...
## 17. Final Resume Decision And Review 04 Disposition

This v5 iteration consumes the contract-production authority in `TASK2_CONTRACT_RESUME_DECISION_V2.md`.

Contract Review 04 is closed without widening Task 2:

- the final sentinel literal is `.oh-my-harness/.operation-in-progress.json`, and every publication, scan, schema, deletion, success, and fault rule binds that path;
- the canonical plan has complete hash field grammars, a closed nine-value conflict enum, no self-hash, and one external confirmation/apply envelope;
- package validation has exactly one executable entry, `npm run validate:package`, with child-status preservation, local-archive smoke, finally cleanup, cleanup-failure handling, and repository-residue checks;
- `test/lifecycle/package-validation.mjs` is the only newly authorized path beyond the prior Task 2 persistent write universe.

The focused reviewer may block only on a traceable requirements/AC/permission/safety defect. Alternative implementation preferences and speculative hardening are non-blocking. Strict PASS authorizes byte-identical accepted-boundary freeze and Task 2 execution under this contract. Valid FAIL returns retry domain `universal-harness-adoption-task-2-lifecycle-contract-20260712-rd1` to `EXHAUSTED` and requires a main-thread split assessment.
