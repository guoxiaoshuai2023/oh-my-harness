# Task 4 Release Readiness Contract v2

Identity: `universal-harness-adoption-task-4-contract-20260714-v2`
...
### Applicable Router And Gate Binding

The future executor task packet must cite this contract and explicitly bind the following current read-only authority. Route discoverability alone is not sufficient.

| Authority | SHA-256 | Trigger and required effect |
| --- | --- | --- |
| `AGENTS.md` | `e5f3646813b10977a9ca93f1abae7d5b34b23b472465a740098bc146b5eadf0d` | Repo router and always-on hard gates; controls every Task 4 action. |
| `task-docs/_harness/adaptive-orchestration-protocol.md` | `8b89c09c4001abd677b6a284a74ad292e62dea94b799af402b71411c63f8572a` | Delegated write, route binding, stable boundary, topology, independence, retry, and main-thread ownership. |
| `docs/agent-routing/current-state-evidence.md` | `8fff20d09f5265a54a915125b9ac999534711863836307a45900ef9f47991364` | Package contents, current dirty state, accepted hashes, and generated/packed outputs materially affect correctness. |
| `docs/agent-routing/task-planning-scale.md` | `29d6c65557757b0ac963f18fe630551d8768c71e1c03fb99bf40bdc3fac7e81c` | Task 4 is the accepted delivery unit; gates and twelve fixtures must not become new delivery tasks. |
| `docs/agent-routing/implementation-boundary.md` | `d5b71bf4bde3625514b67997c572ec03f7f564fe99fde8077839887918aed9e8` | Exact allowed/protected writes, no dependency, no upstream repair, and minimal diff. |
| `docs/agent-routing/validation-and-reporting.md` | `6c66815595a0807aa95d8ba3a424639127bb46b3920cc12f0e9a061611dc8a7b` | Direct behavior evidence, truthful Implementation Report, cleanup, and final reporting. |
| `task-docs/_harness/semantic-fidelity-protocol.md` | `c067e71f884be71b2875bfceaaec6945a1f92d9a4d5ebe250a5f62cf89b3b3c4` | HIGH semantic-risk package transformation, modal-force preservation, AC-pass-but-user-fail, and producer-independent semantic review. |

The external-systems/secrets and high-impact-action routes are not fact-triggered by this authorized implementation: it is limited to local synthetic fixtures, local temporary package inspection, and repository files in Section 8. Registry access, publication, target-repository writes, credentials, destructive external actions, and irreversible operations are forbidden. Discovery of a need for any such action stops the task; the executor cannot select an additional route or enlarge authority itself.

### Gate Ownership

| Gate | Owner and required evidence | Decision authority |
| --- | --- | --- |
| Stable execution boundary | Main thread freezes this contract and a launch baseline; executor verifies identity/hash, branch, dirty state, allowed paths, and protected hashes before writing. | Main thread authorizes launch; any mismatch is STOP. |
| Current-state/protected-path gate | Main thread records the full launch inventory; executor records pre/post status, accepted hashes, changed paths, and residue. | Executor stops on mismatch; evaluator and main thread independently verify. |
| Semantic fidelity gate | Executor produces primary package/router/profile/modal-force and AS-10 evidence without claiming structural proof is semantic proof. | Fresh producer-independent evaluator decides strict PASS/FAIL; main thread makes final acceptance. |
| Validation/reporting gate | Executor runs Section 13 and returns a truthful report with command status and limitations. | Result evaluator checks actual files/output; main thread persists evidence and decides acceptance. |
| Producer independence gate | Executor cannot review its own result; Task 3's waiver cannot satisfy Task 4. | One fresh read-only Result Evaluator, followed by separate main-thread review. |
| Upstream defect gate | Executor identifies evidence and stops without repairing Task 1/2/3. | Main thread routes to the owning task and performs transitive impact analysis. |

Executor nested delegation is `PROHIBITED`. The executor cannot launch another agent, choose or reinterpret routes, change topology, waive a gate, review its own result, or edit this boundary. The main thread owns orchestration, finding classification, synthesis, intervention, and final acceptance.
...
| `AS-06` Modified managed file | Byte-exact target-owned backup is verified against pre-change managed bytes before destructive mutation and again before success; modified path/backup are disclosed, explicit confirmation precedes clean replacement, and later user deletion of the backup is non-blocking and never causes recreation. |
| `AS-07` Unowned collision | Install-time unmanaged `.oh-my-harness/` namespace, unowned prefixed profile, and exact planned-path dirty/untracked overlap each stop with zero writes; update newly-required-path collision also stops with zero writes instead of adoption or replacement. |
| `AS-08` Damaged marker | Missing-half, duplicate, nested, reversed, damaged, and unowned/lookalike marker states each stop with byte-identical `AGENTS.md`; no case may guess, normalize, repair, or emit a derivative `IO_UNAVAILABLE` result. |
...
| `AS-11` Containment | Empty, absolute, NUL-containing, traversal, symlink-parent, and special-file paths stop without outside-root read/write/delete; force or confirmation cannot bypass normalization, containment, ownership, or path-type checks. |
...
- `T4-AC2`: one matrix maps AS-01 through AS-12 to Task ACs, complete discriminating fixture setup, expected decision, primary evidence, and failure signals; scenarios remain fixtures, not delivery tasks. A generic collision, one backup hash, a subset of marker/path variants, or a scenario ID cannot satisfy the complete AS-06/07/08/11 definitions in Section 11.
- `T4-AC3`: AS-01 through AS-09, AS-11, and AS-12 each have direct isolated lifecycle evidence, including update replace/remove/create; backup verification before mutation and before success plus deleted-backup non-recreation; install namespace/profile/dirty and update-new-path collisions; all marker variants with no repair; state-last failure; full containment including NUL and force-bypass rejection; and incomplete-operation follow-up.
...
- the launch task packet omits an applicable route/gate binding or nested-delegation prohibition;
