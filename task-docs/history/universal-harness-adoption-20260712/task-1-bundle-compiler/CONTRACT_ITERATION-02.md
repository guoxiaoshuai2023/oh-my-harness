...
Contract identity: `universal-harness-adoption-task-1-contract-20260712-v2`
...
Current-state evidence captured on 2026-07-12 from the official Node.js release page establishes Node 24 as LTS; the local validation runtime is Node `v24.14.0`. Task 1 therefore freezes `nodeEngine` as `>=24 <25`. Changing this engine requires a revised contract.
...
The inventory never records or hashes itself, never includes `install-state.json`, and never claims backup ownership. In `bundle-map.json`, `calibrationBinding` has exactly `sourcePath`, `destinationPath`, and `dependentAssetIds`; `managedBlock` has exactly `sourceRouter`, `blockTemplate`, `assetPath`, `startMarker`, and `endMarker`. The `rewrites` array consists only of closed `{ "from", "to" }` objects.
...
2. `{ "from": "task-docs/_harness/templates/", "to": ".oh-my-harness/templates/" }`;
3. `{ "from": "docs/agent-routing/", "to": ".oh-my-harness/docs/agent-routing/" }`;
4. `{ "from": "scripts/", "to": ".oh-my-harness/scripts/" }`;
5. six generic filename rules, in planner/plan-evaluator/solution-designer/solution-evaluator/executor/result-evaluator order, from `harness-<role>.toml` to `oh-my-harness-<role>.toml`;
6. `{ "from": "task-docs/_harness/", "to": ".oh-my-harness/" }`.

The compiler applies this array exactly in listed order. Duplicate `from` values with a different `to` value are invalid. Exact-path rules therefore always precede generic prefix and filename rules.

All concrete rules, their exact `from`/`to` strings, and their order are checked into `bundle-map.json`; no code-owned hidden replacement is allowed. After transformation, reference-bearing runtime content must contain none of these source-only prefixes or exact paths: `docs/agent-routing/`, `docs/architecture.md`, `docs/adapters/`, `docs/adoption/`, `examples/minimal-router/`, `task-docs/_harness/`, `.codex/agents/harness-`, `.codex/config.example.toml`, or source helper paths. Source identifiers may remain only in inventory provenance and build evidence. Unknown source-only runtime references fail the build. Build-time source paths must not authorize install-time source access.
