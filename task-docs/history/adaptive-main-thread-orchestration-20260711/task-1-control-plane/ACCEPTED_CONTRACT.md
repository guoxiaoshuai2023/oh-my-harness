...
- Status: Contract iteration 03; implementation is not authorized until strict independent PASS, freeze, boundary-manifest creation, and immutable launch-hash verification
- Contract identity: `adaptive-main-thread-orchestration-task-1-contract-20260711-v3`
- Replaces: `adaptive-main-thread-orchestration-task-1-contract-20260711-v2`
- Previous iteration path/hash: `CONTRACT_ITERATION-02.md` / `607415f1f45a58a830260b0a91cb0a1166446512e6a152911ae6385c019e912b`
...
- Define all Task 1-owned control-plane invariants from the frozen baseline, including topology inputs and revision, delegation value and diminishing returns, non-exhaustive orchestration-mode examples, route/gate binding, no-fan-out default, bounded nested delegation, dependencies, cancellation, synthesis, evidence arbitration, stable write boundaries, context threshold, failure classification and validity, quality and operational retry budgets, intervention, retry domains, stop states, safety recovery, controlled resume, final reporting, and producer-independent verification.
- Keep orchestration modes as composable, non-exhaustive examples. Do not replace the fixed v2 flow with a closed taxonomy, decision tree, or another mandatory pipeline.
...
### Contract And Boundary Control Artifacts

The first contract iteration already exists and is protected:

| Path | Before SHA-256 |
| --- | --- |
| `task-docs/history/adaptive-main-thread-orchestration-20260711/task-1-control-plane/CONTRACT_ITERATION-01.md` | `7e7b19c9f945cfc0a805a194ae363d701ed04952a5e50574f4807b5df24cc9e2` |

The current iteration cannot embed its own final hash, and review/accepted-boundary artifacts do not yet exist. Before implementation, the main thread must create:
...
`TASK1_BOUNDARY_MANIFEST.md` must freeze the exact pre-implementation inventory, stable identity, canonical path, and SHA-256 of every existing `CONTRACT_ITERATION-*.md`, contract review artifact, and `ACCEPTED_CONTRACT.md`. It must also cite the accepted planning-input manifest. No implementation may begin until the boundary manifest exists and all hashes match.

After creating the manifest, the main thread must compute its final SHA-256. The future executor launch packet, which is outside executor write authority, must supply all of these immutable fields:

- accepted boundary identity, path, and SHA-256;
- boundary manifest identity and path;
- boundary manifest expected SHA-256;
- explicit instruction that the manifest is a read-only execution input;
- instruction to verify the manifest file hash and every inventory entry before the first implementation write and again after validation.

The executor is explicitly authorized to read `TASK1_BOUNDARY_MANIFEST.md` and the manifest-listed control artifacts for integrity verification. It is not authorized to modify them. If the launch packet omits or conflicts with the expected manifest hash, implementation must not start.

After implementation, the manifest file itself and every manifest-listed control artifact must still exist at the same path with the same hash. A manifest-file hash mismatch or a missing, added, deleted, renamed, or content-changed contract/review/accepted-boundary artifact is a FAIL. The new adaptive protocol is an implementation file, not a control artifact, and is the only authorized new file outside the pre-implementation control-artifact inventory.
...
7. Produce a complete Task 1-owned requirement-to-heading/evidence map, modal-force preservation table, focused router tabletop results, non-exhaustive-mode negative check, and truthful-reporting positive/negative fixture results in the Implementation Report.
...
One adaptive protocol owns all topology and intervention invariants assigned to Task 1 by the frozen requirements and plan.

Evidence:

- complete requirement-to-heading/evidence map for every Task 1-owned invariant, covering at least topology inputs and revision, delegation value and diminishing returns, composable non-exhaustive modes, optional stages versus mandatory gates, route binding, no-fan-out and bounded nesting, stable boundaries and escalation triggers, context threshold, parallel ownership/dependencies/cancellation/synthesis, evidence arbitration without voting, failure classification and valid-failure rules, stage-local and retry-domain cumulative quality counters, operational-attempt cause budgets, intervention, retry-domain identity, exhaustion/blocking, independent-branch reasoning, safety recovery, controlled resume, producer-independent verification, main-thread final acceptance, and truthful final reporting;
- source requirements section/line or stable heading for each mapped invariant, target protocol/router heading, expected evidence, and status;
- negative closed-taxonomy check showing that modes are examples and a valid new topology can be selected without changing an exhaustive mode list;
...
- completed positive reporting fixture `T1-RP-01` from Section 11;
- completed negative reporting fixture `T1-RP-02` from Section 11 rejecting fabricated role, validation, review, or external-action claims.
...
These fixtures test T1-AC7 protocol behavior. They belong in the Implementation Report and do not authorize a report-template edit.

| Case | Scenario | Required report behavior | Failure signal |
| --- | --- | --- | --- |
| `T1-RP-01` positive proportional report | A MEDIUM Semantic Risk change used one focused executor and a producer-independent semantic reviewer; planner, solution designer, and full result-QA artifacts were intentionally omitted because the main thread supplied a stable boundary and selected targeted evidence. Runtime validation was partly blocked. | State the selected topology and why; identify delegated work and main-thread integration; name primary evidence inspected; identify intentionally omitted major stages and rationale; state the blocked/unvalidated area; report blocker and residual risk without claiming completion beyond evidence. | Report hides omitted stages, omits unvalidated behavior, or converts partial evidence into an unqualified completion claim. |
| `T1-RP-02` fabricated-claim negative | A direct main-thread documentation edit used no subagent, no external system, and only structural validation. | Report only the direct edit and actual structural checks; explicitly avoid claiming reviewer PASS, runtime validation, external readback, or external action. | Report invents a role, independent review, runtime test, external write/readback, or other evidence that did not occur. |
...
test -f task-docs/history/adaptive-main-thread-orchestration-20260711/task-1-control-plane/TASK1_BOUNDARY_MANIFEST.md
...
- recompute `TASK1_BOUNDARY_MANIFEST.md` itself and require an exact match with the expected SHA-256 supplied by the immutable executor launch packet before the first implementation write and after validation;
- recompute every entry in `TASK1_BOUNDARY_MANIFEST.md` and require an exact inventory/path/hash match before and after implementation;
...
- execute and record the non-exhaustive-mode negative check and both truthful-reporting fixtures;
...
- completed non-exhaustive-mode negative check and `T1-RP-01`/`T1-RP-02` results;
- pre/post `TASK1_BOUNDARY_MANIFEST.md` inventory and hash comparison for every contract, review, and accepted-boundary artifact;
- immutable launch-packet expected manifest SHA-256 and the manifest file's actual pre/post SHA-256;
...
- `TASK1_BOUNDARY_MANIFEST.md` is missing, incomplete, mismatched, or does not enumerate every pre-existing contract iteration, final review, and accepted boundary;
- the immutable executor launch packet omits the accepted-boundary identity/hash or boundary-manifest identity/path/expected SHA-256;
- the actual boundary-manifest file SHA-256 differs from the expected SHA-256 in the executor launch packet before or after implementation;
- a manifest-listed control artifact is added, removed, renamed, or content-changed during implementation;
...
5. the main thread creates `TASK1_BOUNDARY_MANIFEST.md` with the exact inventory/path/hash of every contract iteration, the final contract review, and `ACCEPTED_CONTRACT.md`, then independently verifies every entry;
6. the main thread computes the final boundary-manifest SHA-256 and preserves it outside executor write authority for the future launch packet.

After freeze, the executor may read the accepted boundary as Task 1 execution authority, the frozen planning inputs and routed sources it cites, and `TASK1_BOUNDARY_MANIFEST.md` plus its listed control artifacts solely for integrity verification. The executor launch packet supplies the accepted-boundary and boundary-manifest expected hashes but grants no additional write or design authority. The executor must not edit the accepted boundary, boundary manifest, manifest-listed control artifacts, or any contract iteration.
