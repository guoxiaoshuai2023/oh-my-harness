...
- Status: Contract iteration 02; implementation is not authorized until strict independent PASS, freeze, and boundary-manifest verification
- Contract identity: `adaptive-main-thread-orchestration-task-1-contract-20260711-v2`
- Replaces: `adaptive-main-thread-orchestration-task-1-contract-20260711-v1`
- Previous iteration path/hash: `CONTRACT_ITERATION-01.md` / `7e7b19c9f945cfc0a805a194ae363d701ed04952a5e50574f4807b5df24cc9e2`
...
Iteration 02 cannot embed its own final hash, and review/accepted-boundary artifacts do not yet exist. Before implementation, the main thread must create:
...
After implementation, every manifest-listed control artifact must still exist at the same path with the same hash. A missing, added, deleted, renamed, or content-changed contract/review/accepted-boundary artifact is a FAIL. The new adaptive protocol is an implementation file, not a control artifact, and is the only authorized new file outside the pre-implementation control-artifact inventory.
...
- recompute every hash in `TASK1_BOUNDARY_MANIFEST.md` and require an exact inventory/path/hash match before and after implementation;
