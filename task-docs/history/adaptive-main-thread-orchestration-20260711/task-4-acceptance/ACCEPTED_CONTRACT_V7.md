# Revised Execution Contract: Task 4 v7 - Restore Complete-v2 Selection Authority

## Contract Identity

- Task identity: `adaptive-main-thread-orchestration-task-4-acceptance-v7`
- Contract iteration: `07`
- Stable identity: `adaptive-main-thread-orchestration-task-4-contract-20260711-v7`
...
5. This v7 execution contract.

The v5 contract, Implementation Report, and Result QA are protected failure evidence and current-state provenance only. They are not accepted behavioral authority. Where v5 says full v2 is preserved `only` for explicit request or stricter policy, this v7 contract rejects and supersedes that statement.

## Triggered Routes And Gates

Binding mode is `explicit binding`; repository discoverability alone is invalid. The main thread owns the binding and must include these exact paths and gates in the future executor and evaluator packets. The executor may apply them but may not select, omit, reinterpret, or add routes.

| Exact authority path | Factual trigger and use | Gate owner | Required evidence | Decision point |
| --- | --- | --- | --- | --- |
| `AGENTS.md` | Repo router, always-on hard gates, adaptive control-plane entry, and route trigger source | main thread | verified path/hash and explicit packet binding | before launch and final acceptance |
| `docs/agent-routing/README.md` | RSM-005, trigger-map maintenance, and routed-doc versus evidence-artifact distinction | main thread; result evaluator independently checks | updated trigger/route row, smoke result, and manual semantic comparison | before accepting T4-AC1A |
| `task-docs/_harness/adaptive-orchestration-protocol.md` | Harness topology, deliberate selection, intervention, retry, cancellation, synthesis, and producer independence | main thread | three-state modal comparison, six-field deliberate record, gate/owner/evidence map | topology fixture decision and final acceptance |
| `docs/agent-routing/current-state-evidence.md` | Rejected v5 bytes, complete dirty worktree, current public docs, and pre/post evidence determine correctness | main thread before launch; executor reports; evaluator independently checks | v6 baseline, seven pre/post hashes, status/untracked inventory, freshness and changed-path evidence | before first write, after validation, and Result QA |
| `docs/agent-routing/task-planning-scale.md` | One Task 4 revision delivery unit; ACs, AS rows, CC fixtures, review, and semantic checks are gates/evidence rather than new delivery tasks | main thread | one-task scope, no AS-32, no scenario-as-task or review-only task | contract acceptance and scope review |
| `docs/agent-routing/implementation-boundary.md` | Documentation/template/calibration revision with exact seven writes and no unrelated refactor/dependency | main thread freezes; executor obeys; evaluator verifies | allowed/protected inventory, seven before/after hashes, minimal diff | before write and Result QA |
| `docs/agent-routing/validation-and-reporting.md` | Fresh T4 AC evidence, v2 report/QA, smoke limitation, truthful current-state reporting | executor supplies producer evidence; result evaluator decides independently; main thread accepts | commands, semantic reviews, unchecked areas, report/QA identities, strict verdict | producer completion, Result QA, final acceptance |
| `task-docs/_harness/semantic-fidelity-protocol.md` | HIGH semantic revision, original authority conflict, AC-pass-but-user-fail, independent result judgment | main thread owns Anchor/Outcome; result evaluator is producer-independent | requirements/protocol excerpts, three-state Outcome Contract, CC-DELIBERATE and negative evidence | before execution boundary freeze and semantic Result QA |
| `docs/agent-routing/external-systems-and-secrets.md` | AS-05, AS-21, and AS-28 through AS-31 contain synthetic external-system/current-state facts only | fixture author records; evaluator checks classification | explicit synthetic inputs, no-live-action statement, expected STOP/readback/tabletop decisions | each cited AS regression decision |
| `docs/agent-routing/high-risk-actions.md` | AS-05 destructive fixture, AS-24 permission fixture, and AS-28 recovery fixture require exact synthetic safety semantics | fixture author records; evaluator checks independently | target/confirmation/recovery/readback or permission evidence fixed in frozen AS rows | each cited AS regression decision |

External-system and high-risk routes authorize synthetic tabletop reasoning only. No network, credential, secret, permission mutation, delete, rollback, confirmation, recovery, readback, or other real action is active or authorized. Actual external writes, secret handling, publish/deploy, billing/payment, key operations, dependencies, and destructive repository operations are fact-triggered routes considered but inactive because this revision edits local Markdown only. Any newly discovered real trigger requires STOP and a revised boundary.

Triggered safety/semantic gates are mandatory regardless of selected workflow roles. `CC-DELIBERATE-FULL-V2` may select all six interfaces, but that fixture does not change route ownership, authorize live execution, or make the complete sequence a default.

## Frozen Failure And No-Impact Inputs

| Artifact | Identity | Path | SHA-256 | Use in v7 |
| --- | --- | --- | --- | --- |
...
Add exactly this field-complete case under the existing `Adaptive Orchestration Calibration Cases` schema:
...
The prompt/template targets identify calibration consumers and comparison surfaces; they do not expand v7 write authority. Only the routing-scenario template is writable under this contract.

Calibration detects recurring failure; it does not replace primary authority or semantic review.

## Scope And Write Authority

Only these seven implementation paths may be modified by a future v7 executor:
...
- all v5/v6/v7 contracts, reviews, reports, QA, decisions, manifests, and baselines.

No validator, dependency, seventh role, nested delegation, external action, secret access, new/additional workflow stage outside the separately authorized future v7 executor -> read-only Result QA -> main-thread decision, commit, push, or PR is authorized. If a protected change appears necessary, STOP and return to contract design; do not expand this boundary.
...
10. Preserve all protected/current-state evidence and return a versioned v7 revision Implementation Report response only.
...
All ACs require fresh v7 evidence. A structural PASS, v5 report statement, or v5 scenario PASS cannot satisfy semantic acceptance.

## Validation

Before any future write, verify the v7 accepted contract, v7 manifest, v7 launch baseline, v6 contract-state baseline, failure decision, all protected inventory entries, branch/refs, and seven before hashes. After changes, run at minimum:
...
- v5 failed artifacts and all v6/v7 gate artifacts unchanged;
- structural validator described only as smoke/mention coverage.

## Upstream Finding Routing

If v7 work exposes a Task 1, Task 2, or Task 3 defect, STOP before modifying the protected path. Record the finding and owning task, issue a new versioned upstream boundary, perform transitive impact analysis, supersede and reaccept affected downstream results or record specific no-impact evidence, stabilize all upstream identities, and strictly review a new Task 4 boundary before resuming.
...
- a validator/dependency/seventh role/nested delegation/external action/secret/new or additional workflow stage outside the contracted future executor -> read-only Result QA -> main-thread decision/commit/push/PR is required.
...
Persist the iteration review as `task-docs/history/adaptive-main-thread-orchestration-20260711/task-4-acceptance/CONTRACT_REVIEW_ITERATION-07.md`, identity `adaptive-main-thread-orchestration-task-4-contract-review-iteration-07-20260711-v1`. It must explicitly decide the three-state model, deliberate-selection evidence threshold, no-fixed-pipeline rule, revised AC/fixture coverage, v5 dependency treatment, Task 1-3 no-impact, scope, route/gate binding, and protected evidence.

On strict PASS only, freeze byte-identically as:

- accepted boundary: `task-docs/history/adaptive-main-thread-orchestration-20260711/task-4-acceptance/ACCEPTED_CONTRACT_V7.md`, identity `adaptive-main-thread-orchestration-task-4-contract-20260711-v7`;
- final review: `task-docs/history/adaptive-main-thread-orchestration-20260711/task-4-acceptance/CONTRACT_REVIEW_V7.md`, identity `adaptive-main-thread-orchestration-task-4-contract-review-20260711-v7`;
- boundary manifest: `task-docs/history/adaptive-main-thread-orchestration-20260711/task-4-acceptance/TASK4_BOUNDARY_MANIFEST_V7.md`, identity `adaptive-main-thread-orchestration-task-4-boundary-manifest-20260711-v2`.

A later, separately authorized executor launch must first freeze `EXECUTOR_LAUNCH_BASELINE_V7.md`, identity `adaptive-main-thread-orchestration-task-4-executor-launch-baseline-20260711-v2`.
