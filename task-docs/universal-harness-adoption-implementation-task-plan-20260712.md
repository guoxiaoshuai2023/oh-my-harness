...
Plan identity: `universal-harness-adoption-implementation-plan-20260712-v3`

Status: `PLAN BASELINE - accepted only through the frozen planning-input manifest after strict Pass B`

Canonical path: `task-docs/universal-harness-adoption-implementation-task-plan-20260712.md`
...
- Staged delta: empty
- Untracked files before this planning task: 24, comprising the v5 requirements and superseded adoption history
...
| Scripts | Three stdlib Python helpers; smoke validators disclaim semantic proof. | Ship under `.oh-my-harness/scripts/`, rewrite documentation paths, compile, and preserve their evidence limitations. |
...
- local package identity `oh-my-harness` and package binary `oh-my-harness`;
...
- exact version-pinned command forms `npx --yes oh-my-harness@<version> install --target <repo>`, `npx --yes oh-my-harness@<target-version> update --target <repo>`, and `npx --yes oh-my-harness@<compatible-version> uninstall --target <repo>`;
- stable-channel forms using `oh-my-harness@latest`, with the resolved immutable package/bundle version recorded before target writes;
- local acceptance forms using the packed archive, such as `npx --yes ./oh-my-harness-<version>.tgz <operation> --target <repo>`.
...
The local package name, binary, lifecycle subcommands, and command relationships above are fixed implementation inputs. Use of `oh-my-harness` on a public registry is not an ownership claim: registry availability/ownership, license, signing, credentials, and publication remain outside implementation acceptance. If the owner cannot authorize that public name, publication stops for a new user decision and corresponding package/docs rebuild; implementation must not silently publish under another name.
...
- `T1-AC1`: the checked-in package contract fixes local package identity `oh-my-harness`, binary `oh-my-harness`, binary destination owned by Task 2, version-pinned/stable-channel/local-archive command relationships, explicit package allowlist requirements, and zero third-party runtime dependencies.
...
- `T1-AC4`: inventory includes version, compatibility, required/optional classification, source provenance mapping, exact destinations, expected transformed hashes, ownership surface, and managed-block identity; it excludes mutable state and avoids self-hash.
...
- Python compile and TOML parse checks on transformed assets;
...
- `T2-AC19`: the real `package.json`, lockfile, binary entry, and lifecycle scripts conform exactly to the accepted Task 1 package contract, run without third-party runtime dependencies, and make the local packed-archive command runnable.
...
- `T3-AC1`: README presents the stable package command surface as the primary path; manual source copying is not presented as equivalent managed installation.
...
- `T3-AC8`: local implementation acceptance ends at reproducible package/fixture/CI evidence; license, registry identity/ownership, credentials, signing, and actual publication require a later owner decision and external-write boundary.
...
- `T4-AC1`: CI retains every existing source-Harness check and adds supported Node setup, package build, package-content validation, lifecycle tests, and fixture coverage without third-party runtime dependencies.
...
- `T4-AC6`: final transformed payload has complete reference closure, valid six TOMLs, compiled Python helpers, consistent calibration binding, and no source-only runtime path.
...
- `T4-AC9`: README, architecture, adapter, lifecycle spec/runbook, package CLI help, protocols, templates, calibration, and CI use consistent names, paths, authority, evidence limits, and publication boundary.
...
| --- | --- | --- |
| AS-01 Empty repo install | T1-AC2..T1-AC8; T2-AC2..T2-AC4; T2-AC19; T4-AC3 | Runnable local package, complete target tree, payload/block/state verification, ownership inventory. |
...
| AS-05 Clean update | T2-AC7; T4-AC3 | One replacement, one removal, one absent-only creation, block/state/reference verification. |
...
| AS-09 Safe uninstall | T2-AC11; T2-AC14..T2-AC15; T4-AC3 | Ownership/target identity/state validation, deletion set, target preservation, state-last ordering and injected state-delete failure. |
...
- select or assert ownership of an unverified public package name;
...
Actual open-source publication requires a later owner decision on license and registry identity plus an exact high-impact external-write boundary, dry-run/preview, confirmation, and producer-independent verification. A locally valid archive is release-ready evidence, not proof of publication.

## 12. Decisions Left To Single-Task Boundaries

The plan fixes npm, local package name/binary `oh-my-harness`, the version-pinned/stable-channel/local-archive command relationship, the standard-library implementation constraint, mapping model, lifecycle semantics, and task ownership. Individual accepted boundaries may still decide:
...
- public package identity or license is treated as implicitly authorized;
