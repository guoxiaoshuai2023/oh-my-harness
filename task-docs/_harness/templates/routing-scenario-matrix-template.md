...
| `RSM-005` | `task-docs/` plan execution | `docs/agent-routing/task-planning-scale.md`; `task-docs/_harness/semantic-fidelity-protocol.md`; `docs/agent-routing/validation-and-reporting.md`. | Follow planner -> plan evaluator -> solution designer -> solution evaluator -> frozen `ACCEPTED_CONTRACT.md` -> executor -> result evaluator -> main-thread review. | Stop if executor scope is unclear, attempts a later task, edits outside accepted contract, or turns harness gates into delivery tasks. | Accepted contract identity, implementation report, AC evidence, result QA, and dirty-worktree preservation when required. | External-system, high-risk, current-state, and implementation routes unless their triggers appear in the accepted contract. |
